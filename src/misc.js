...
export async function onSetShareVision(data){
  ...
  if (data.globalSharedVision != undefined) {
    ...
    else if (data.globalSharedVision == 'toggle') globalSharedVision = !game.settings.get(moduleName,'enable');
    if (globalSharedVision != game.settings.get(moduleName,'enable')) {
      ...
      ui.controls.controls.find(controls => controls.name == "token").tools.find(tools => tools.name == "enableSharedVision").active = globalSharedVision;
      ui.controls.render();
    }
  }
  if (data.disableAll != undefined) {
    ...
    else if (data.disableAll == 'toggle') disableAll = !game.settings.get(moduleName,'disableAll');
    ...
    if (disableAll != game.settings.get(moduleName,'disableAll')) {
      ...
      ui.controls.controls.find(controls => controls.name == "token").tools.find(tools => tools.name == "disableAllSharedVision").active = disableAll;
      ui.controls.render();
      updateAllTokens();
    }
  }
...
export async function initializeSources(updateSource = false){
  ...
  if (compatibleCore('10.0')) {
    ...
    for (let token of tokens) {
      ...
      if (actor.getFlag('SharedVision','enable') || userSetting?.vision || getOverride('vision',token)) {
...
export function isSharedVision(token) {
  ...
  if (token.actor != null) {
    ...
    if (game.settings.get(moduleName,'enable')) {
        sharedVision = token.actor.getFlag("SharedVision","enable") != undefined ? token.actor.getFlag("SharedVision","enable") : false; 
    }

    if (sharedVision == false) {
      let userSetting = token.actor.getFlag('SharedVision','userSetting');
...
export function getOverride(type,token) {
  ...
  const d = compatibleCore('10.0') ? token.document.disposition : token.data.disposition;
...
export function revealTokenFog(token) {
  ...
  if (compatibleCore('10.0')) {
    ...
    if (visionSource.los == undefined) visionSource.los = {isContrained: true};
    ...
    token.vision.initialize({
      x: origin.x,
      y: origin.y,
      dim: Math.clamped(token.getLightRadius(token.data.dimSight), 0, d.maxR),
      bright: Math.clamped(token.getLightRadius(token.data.brightSight), 0, d.maxR),
      angle: token.data.sightAngle,
      rotation: token.data.rotation
    });

    const sightLayer = canvas.layers.find(l => l.options.name === 'sight');
