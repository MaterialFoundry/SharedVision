...
Hooks.on("TriggerHappy", (key, args) => {
    if (key == "SharedVision") {
        ...
        if (args == "enable") enable = true;
            ...
            enable = !game.settings.get(moduleName, "enable");
...
export function heyWait_onTrigger(sceneId, tileId) {
    ...
    let enable = false;
    if (tile.data?.flags?.["sharedVision"]?.enabled)
        enable = tile.data?.flags?.["sharedVision"]?.enabled;

    if (enable != game.settings.get(moduleName, "enable")) {
        shareVision(enable);
        ui.controls.controls
            .find((controls) => controls.name == "token")
            .tools.find((tools) => tools.name == "enableSharedVision").active =
            enable;
        ui.controls.render();
    }
}
...
export function heyWait_onTileHud(tileHud, html) {
    ...
    let enabled = false;
    if (tile.data?.flags?.["sharedVision"]?.enabled)
        enabled = tile.data?.flags?.["sharedVision"]?.enabled;
