...
export function migrateSettings() {
  if (!game.settings.get(moduleName,'migration_v1.0.10')) {
    console.log('Shared Vision - Migrating Shared Vision settings to v1.0.10')
...
export class configMenu extends FormApplication {
  ...
  getData() {
    ...
    const permissions = [
      {id: 'none', name: game.i18n.localize(compatibleCore('10.0') ? "OWNERSHIP.NONE" : "PERMISSION.NONE"), permissions: getOverridePermissions('none')},
      {id: 'limited', name: game.i18n.localize(compatibleCore('10.0') ? "OWNERSHIP.LIMITED" : "PERMISSION.LIMITED"), permissions: getOverridePermissions('limited')},
      {id: 'observer', name: game.i18n.localize(compatibleCore('10.0') ? "OWNERSHIP.OBSERVER" : "PERMISSION.OBSERVER"), permissions: getOverridePermissions('observer')},
      {id: 'owner', name: game.i18n.localize(compatibleCore('10.0') ? "OWNERSHIP.OWNER" : "PERMISSION.OWNER"), permissions: getOverridePermissions('owner')},
    ];
  ...
  async _updateObject(event, formData) {
    ...
    for (const [key, value] of Object.entries(formData)) {
      ...
      if (split[0] == 'combatSelect') {
        ...
        config[split[0]][split[2]][split[1]] = value;
    }
